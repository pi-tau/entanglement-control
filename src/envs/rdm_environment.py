...
class QubitsEnvironment:
    ...
    def __init__(self, num_qubits=2, epsi=1e-4, batch_size=1):
        ...
        self._states = np.zeros(self.shape, dtype=np.complex128)
        self._states_swap_buffer = np.zeros(self.shape, dtype=np.complex128)
        self._entropies_cache = np.zeros((self.batch_size, self.L), dtype=np.float64)
        self.reset()

    def reset(self):
        """Prepare all states in the batch as | 000..00 >"""
        psi = np.zeros((self.batch_size, 2 ** self.L), dtype=np.complex128)
        psi[:, 0] = 1.0
    ...
    def step(self, actions):
        ...
        util._transpose_batch_inplace(batch, qubit_indices, L)
        ...
        # Fixes the bug occuring on applying the same action twice
        rdms[np.abs(rdms) < 1e-5] = 0
        # ----
