...
class ILAgent(BaseAgent):
    ...
    def train(self, dataset, num_epochs, batch_size, learning_rate, lr_decay=1.0,
        ...
        for i in tqdm(range(num_epochs)):
            ...
            if i % test_every == 0:
                ...
                tic_true = time.time()
                true_acc = self.test_true_acc(steps=steps, num_test=10, logfile=logfile)
                toc_true = time.time()
                self.test_history[i] = {
                    "entropies" : entropies,
                    "returns"   : returns,
                ...
                logText(f"    True classification took: {toc_true-tic_true:.3f} seconds", logfile)
                logText(f"    True action classification: {true_acc:.3f}", logfile)
                logText(f"    Train set classification accuracy: {accuracy:.3f}", logfile)
    ...
    def test_true_acc(self, steps, num_test, logfile):
        _env = QubitsEnvironment(num_qubits=self.env.L, epsi=self.env.epsi)
        expert = SearchExpert(_env, beam_size=100)
        batch_size = self.env.batch_size  # number of trajectories
        correct, total = 0, 0

        # Begin testing.
        for _ in range(num_test):
            # Generate a rollout using the current agent policy.
            self.env.set_random_states(copy=False)
            states, actions, rewards, done = self.rollout(steps, greedy=False)
            mask = self.generate_mask(done)

            # For every state encountered during all trajectory rollouts use the
            # expert to check whether the action selected from the agent was good.
            for b in range(batch_size):
                for s in range(steps):
                    if done[b, s]: break
                    act = actions[b, s].detach().cpu().numpy()
                    psi = states[b, s].detach().cpu().numpy()
                    psi = (psi[: psi.shape[0] // 2] + 1j * psi[psi.shape[0] // 2 :]).reshape((2,)*self.env.L)
                    _, e_actions = expert.rollout(psi.copy(), num_iter=1000, verbose=False)

                    # If the expert takes the same action, then the action is correct.
                    if act == e_actions[0]:
                        correct += 1
                        continue

                    #
                    _env.states = np.expand_dims(psi, axis=0)
                    phi, _, _ = _env.step([act])
                    _, ee_actions = expert.rollout(np.squeeze(phi, axis=0), num_iter=1000, verbose=False)
                    if len(e_actions) > len(ee_actions):
                        correct += 1
