"""
python3 benchmark_il_agent.py -s 0 -q 5 --env_batch 100 --steps 30 --max_steps 5000 --epsi 1e-3 -t 100 --policy_file policy_200.bin
"""
...
parser.add_argument("--policy_file", dest="policy_file", type=str, default="")
...
log_dir = os.path.join("..", "logs", f"{args.num_qubits}qubits", "benchmark_il_agent")
...
if args.policy_file == "":
    print("Please provide policy model!")
    exit(0)
env = QubitsEnvironment(args.num_qubits, epsi=args.epsi, batch_size=args.env_batch)
policy = FCNNPolicy.load(args.policy_file)
agent = PGAgent(env, policy)
logText("================================================================================", log_file)
logText(f"Testing agent policy: {args.policy_file}", log_file)
